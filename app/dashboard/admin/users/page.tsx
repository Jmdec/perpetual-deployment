  ...
  export default function AdminUsersPage() {
    ...
    const getStatusBadge = (status: string) => {
      const styles = {
        "No-Record": "bg-orange-100 text-orange-700",
      ...
      const icons = {
        "No-Record": <Clock className="w-3 h-3" />,
                          ...
                          <tbody className="divide-y divide-gray-200 bg-white">
                            {users.map((user) => (
                              <tr key={user.id} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-3 sm:px-4 py-3 whitespace-nowrap">{getStatusBadge(user.status === 'approved' ? 'Confirmed' : 'No-Record')}</td>
          ...
          {isConfirmModalOpen && selectedUser && (
            <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-4">
              <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
                {/* Confirmation Header */}
                <div
                  className={`px-6 py-4 ${actionType === "approved" ? "bg-green-600" : actionType === "deactivated" ? "bg-gray-600" : "bg-red-600"
                    } text-white`}
                >
